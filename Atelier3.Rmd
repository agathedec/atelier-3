...
```{r}
nutriage <- read.csv("~/Downloads/Atelier3-master/nutriage.csv")
length(nutriage)
names(nutriage)
```
```{r}
...
pie(table(chocol.f),main = "c. chocolat chez les femmes")
pie(table(nutriage$chocol),main = "c. chocolat chez la population")
```
```{r}
hist(nutriage$poids)
```
